...
namespace JurisTempus.Controllers
{
  ...
  public class TimeBillsController : ControllerBase
  {
    ...
    public async Task<ActionResult<TimeBillViewModel>> Post([FromBody]TimeBillViewModel model)
    {
      ...
      {
        return CreatedAtAction("Get", new { id = bill.Id },
          _mapper.Map<TimeBillViewModel>(bill));
      }
    ...
    [HttpPut("{id:int}")]
    public async Task<ActionResult<TimeBillViewModel>> Put(int id,
      [FromBody]TimeBillViewModel model)
    {
